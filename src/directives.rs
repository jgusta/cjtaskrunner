...
fn execute_directive(
    ...
    match name {
        ...
        "mkdir" => {
            ...
            for path in argv {
                fs::create_dir_all(cwd.current().join(path))?;
            }
            *index += 1;
            Ok(0)
        }
        "cp" => {
            let argv = interpolate_argv(args, &effective_env.vars)?;
            copy_files(cwd.current(), &argv, lines[*index].line_number)?;
            *index += 1;
            Ok(0)
        }
        "cpdir" => {
            let argv = interpolate_argv(args, &effective_env.vars)?;
            copy_dirs(cwd.current(), &argv, lines[*index].line_number)?;
            *index += 1;
            Ok(0)
        }
        "rename" => {
            let argv = interpolate_argv(args, &effective_env.vars)?;
            rename_path(cwd.current(), &argv, lines[*index].line_number)?;
            *index += 1;
            Ok(0)
        }
        "stop" => {
...
fn copy_files(base_dir: &Path, argv: &[String], line_number: usize) -> CjResult<()> {
    if argv.len() < 2 {
        return Err(CjError::new(format!(
...
fn copy_dirs(base_dir: &Path, argv: &[String], line_number: usize) -> CjResult<()> {
    if argv.len() < 2 {
        return Err(CjError::new(format!(
...
fn rename_path(base_dir: &Path, argv: &[String], line_number: usize) -> CjResult<()> {
    if argv.len() != 2 {
        return Err(CjError::new(format!(
